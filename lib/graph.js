module.exports = (function() {
  ...
  class Graph {
    ...
    toBin() {
      ...
      let toBin = function(v) { return v.toBin(); };

      let nodeCollections = this._nodeCollections;
      let edgeCollections = this._edgeCollections;

      let nc = Object.keys(nodeCollections).map(function(v) {
        return nodeCollections[v];
      });
      let ec = Object.keys(edgeCollections).map(function(v) {
        return edgeCollections[v];
      });

      return [].concat(
        [].concat.apply([], nc.map(toBin)),
        [].concat.apply([], ec.map(toBin)),
        [].concat.apply([], this._nodes.map(toBin)),
        [].concat.apply([], this._edges.map(toBin))
      );
